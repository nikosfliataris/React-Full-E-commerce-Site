...
import "./Pc-Accesories.css";
import PcAproducts from "./info-pages/PcAproducts";
import { useStateValue } from "../../ComponentAPI/StateProvider";
function PcAProducts() {
  const [productsPcA, setProductsPcA] = useState([
    /* {
      id: "Samsung S21",
      image:
  ...
  const getProducts = () => {
    db.collection("Pc-Accesories").onSnapshot((snapshot) => {
      setProductsPcA(
  ...
  const handleAdd = (e) => {
    e.preventDefault();
    db.collection("Pc-Accesories").doc(id).set({
      title: title,
      image: image,
      price: price,
      ...
      <div className="products">
        <div className="breadcrumb-column">
          <Breadcrumb className="breadcrumb">
            ...
            <Breadcrumb.Item active>Pc Accesories</Breadcrumb.Item>{" "}
          </Breadcrumb>
        </div>
        <div className="sidebar-container container">
          <Filterbar />
        </div>
        <div id="pca-list-product ">
          {productsPcA.map((product) => (
            <PcAproducts
